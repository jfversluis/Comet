...
namespace Comet.Tests
{
	public class ComponentStateTests : TestBase
	{
		...
		{
			// Reactive<T> should be usable anywhere State<T> is
			var reactive = new Reactive<int>();
			reactive.Value = 42;
			Assert.Equal(42, reactive.Value);

			// Verify it's a State<T>
			Assert.IsAssignableFrom<State<int>>(reactive);
		}
		...
		public void ReactiveImplicitConversion()
		{
			...
			// Should be assignable to State<T> reference
			State<int> state = reactive;
