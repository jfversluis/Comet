...
namespace Comet.Styles
{
	...
	public sealed class Token<T>
	{
		...
		/// <summary>
		/// Implicit conversion to Binding&lt;T&gt;.
		/// Creates a reactive binding that resolves against the GLOBAL theme.
		/// For scoped theme resolution, use view-aware overloads.
		/// </summary>
		public static implicit operator Binding<T>(Token<T> token)
			=> (Func<T>)(() => token.Resolve(ThemeManager.Current()));

		/// <summary>
		/// Projects this token through a transform, returning a reactive Binding.
		...
		public Binding<TResult> Map<TResult>(Func<T, TResult> transform)
			=> (Func<TResult>)(() => transform(Resolve(ThemeManager.Current())));
	}
