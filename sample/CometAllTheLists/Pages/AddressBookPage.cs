...
public class AddressBookPage : View
{
	readonly State<List<Contact>> allContacts;

	public AddressBookPage()
	{
		allContacts = new State<List<Contact>>(new List<Contact>
		{
	...
	View body()
	{
		var sections = allContacts.Value
			.GroupBy(c => c.Section)
			.OrderBy(g => g.Key)
			.ToList();
